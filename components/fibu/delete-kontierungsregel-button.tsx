...
export function DeleteKontierungsregelButton({
  id,
  label,
}: {
  id: string
  ...
  async function onDelete() {
    setLoading(true)
    setError(null)
    const res = await fetch(`/api/fibu/kontierungsregeln/${id}`, {
      method: "DELETE",
    })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Kontierungsregel <strong>{label}</strong> wird dauerhaft gelöscht.
          </DialogDescription>
        </DialogHeader>
