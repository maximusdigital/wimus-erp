...
/**
 * Manuelle Board-Reihenfolge der Deal-Pipeline persistieren: setzt board_sort = Position
 * (0..n) für die übergebenen Deal-IDs (eine Stage). RLS scoped.
 */
export async function POST(request: NextRequest) {
  ...
  const results = await Promise.all(
    parsed.data.ids.map((id, i) =>
      supabase.schema("wimus").from("crm_deals").update({ board_sort: i }).eq("id", id)
    )
  )
