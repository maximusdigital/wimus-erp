...
|-------------|-------|---------|----------|
| 16:10 | 021 org-migration | **Phase-B-Spec angelegt** (v0.1.0 entwurf, #21): mandant→projekt Phase B. Zwei-Achsen-Modell (firma_id rechtlich + projekt_id operativ, Objekt kann Marke wechseln ohne Firma-Wechsel), gesellschaften→firmen 1:1, Anker=objekte (Rest erbt über RLS-Elternkette). Schritte B0 Seed-Fix (ALFA DEVELOPMENT als 4. Marken-Top)→B1 gesellschaften→firmen+FK-Umhängen→B2 projekt-Backfill→B3 Verifikation. Additiv+umkehrbar, RLS unangetastet (Phase C). NICHT gebaut — erst Gegenlesen. | Spec 021_*_phase_b |
| 15:55 | 004 org-hierarchie | **Migration 004 GEBAUT + verifiziert + committet** (Commit 33bb72d, #21 Phase A): Org-Hierarchie workspaces/firmen/projekte als getrackte idempotente Migration (Live-Ist 1:1 nachgezeichnet — 115 Spalten, 7 CHECK-Wertelisten, Indizes/RLS/Trigger, projektmanager-FK konditional). CC-Verifikation gegen Live-Schema: 0 Abweichungen. Schließt „fehlende Migration 004". NICHT gegen Live angewandt (No-Op). | Report 20260629_1555 |
| 14:00 | 010 berechtigungen | **Spec angelegt** (v0.1.0 entwurf, 000/200/300): RBAC Bereich×Stufe×Scope auf vorhandenem rollen/benutzer_rollen-Fundament (Migration 002, 12 Rollen geseedet aber rechtelos). Bereiche pro Modul, Stufen kein/lesen/schreiben/freigeben, Scope global/mandant/gesellschaft/objekt vererbend. 3-Stufen-Rollout (Modell+Engine → RLS modulweise → Gruppen). Vertretung über gueltig_von/bis, DSGVO-Matrix-Export, keine Feldebene. Teilen/Weiterleiten → Backlog #19. Löst perspektivisch Backlog #14 Audit-Rollen. | Specs 010_* |
| 12:25 | 009/008 | **Stufe 2 GEBAUT** (Commit 8934ca7, 407 Tests grün, Review FREIGABE): HistorieTab + Custom-Field-Wert-Editor auf allen 4 Kern-Detailseiten (objekte/einheiten/kontakte/vorgaenge), als Card-Abschnitt via neuem Wrapper components/shared/detail-zusatz.tsx (keine Seite nutzt Tabs). NEU: API app/api/felder/werte (GET listDefs+getWerte, PUT setWert, mandant serverseitig), Editor components/felder/felder-werte.tsx (je feldtyp passendes Element, Pflicht-Validierung, Dirty-Check). Kontakt-Mapping: kontakt_typ=firma→organisation sonst person (Felder), Historie bleibt bezugTyp=kontakt. | Report 20260629_1225 (ex-Backlog #14) |
| 12:10 | 009/008 | **Stufe 2 beauftragt** (ex-Backlog #14 HistorieTab + 008-Detail-UI): HistorieTab auf 4 Kern-Detailseiten einhängen (objekte/einheiten/kontakte/vorgaenge, Komponente fertig) + Custom-Field-Wert-Editor NEU bauen (Service getWerte/setWert steht, UI+API fehlt) + einhängen. Ein Aufwasch über dieselben Seiten. Rest von #14 (weitere Lieferanten, Audit-Rollen, Retention) bleibt offen. | Prompt 20260629_1210 |
| 11:35 | 009 historie | **FiBu-Lieferant GEBAUT** (Commit e99a90d, 401 Tests grün, keine Migration, Review-Subagent FREIGABE): 3 FiBu-Aktivitäten via protokolliere() — zahlung_eingegangen (Bank-Auto-Import + manuelles Zuordnen), mahnung_versandt (Mahnlauf + manuelle Mahnung), beleg_verbucht (KI-Auto + Mensch-Freigabe), je 2 reale Andockstellen. Neu: lib/fibu/historie.ts (Emitter), lib/fibu/k1-bezug.ts (DB-K1-Resolver, teilt parseVerwendungszweck). Service additiv erweitert: primaerBezug optional („nur Mandant"-Aktivitäten), bezuegeAusHierarchie(). Nicht-blockierend (safe()). Im zentralen Feed sichtbar; HistorieTab dezentral = Stufe 2. | Report 20260629_1135 (ex-Backlog #14 Teil 1) |
...
| Datum/Zeit (MESZ) | Vorgang |
|-------------------|---------|
| 2026-06-29 12:25 | Stufe 2 GEBAUT (Commit 8934ca7, 407 Tests, Review FREIGABE): HistorieTab dezentral auf 4 Kern-Detailseiten eingehängt (objekte/einheiten/kontakte/vorgaenge, Card via detail-zusatz.tsx) + 008-Custom-Field-Wert-Editor (API felder/werte, components/felder/felder-werte.tsx). Aktivitäts-Historie jetzt dezentral sichtbar. (ex-Backlog #14) |
| 2026-06-29 11:35 | FiBu-Lieferant GEBAUT (Commit e99a90d, 401 Tests grün, Review FREIGABE): zahlung_eingegangen/mahnung_versandt/beleg_verbucht via protokolliere(), je 2 reale Andockstellen. Neu lib/fibu/historie.ts + k1-bezug.ts (DB-K1-Resolver). Service additiv: primaerBezug optional („nur Mandant"), bezuegeAusHierarchie(). Nicht-blockierend. (ex-Backlog #14 Teil 1) |
...
| Datum | Vorgang |
|-------|---------|
| 2026-06-29 | Serena-MCP (semantische Code-Tools) dazugeschaltet + CLAUDE.md-Regel: CC bevorzugt bei Code-Navigation/Editieren find_symbol/find_referencing_symbols/replace_symbol_body statt Grep/Read (Token-Ersparnis); Ausnahme Doku/Markdown. Hintergrund: CC bevorzugt sonst eingebaute Tools, Serena bleibt ungenutzt. |
