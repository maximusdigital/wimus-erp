...
- In package.json: vitest ^4, @testing-library/react, @playwright/test, supertest,
  @vitest/ui, @vitest/coverage-v8 — alle vorhanden.
- **Umgesetzt (2026-06-27):** vitest.config.ts, playwright.config.ts, tests/-Baum
  (unit/integration/e2e + setup) und npm-Scripts existieren; 32 Testdateien, 259 Tests grün.
  Offen bleibt nur das pgTAP-Aktivieren in der DB (`CREATE EXTENSION pgtap`).
...
| Tool | Zweck | Status |
|------|-------|--------|
| Vitest | Unit + Integration, ESM-native | ✅ vorhanden (inkl. @vitest/ui + coverage-v8) |
...
Setup verbleibend: `npx playwright install chromium` (Browser); `CREATE EXTENSION IF NOT
EXISTS pgtap;` (DB). vitest-Pakete sind bereits installiert.
...
> Kontierungs-Lookup) ergänzen diese Testbasis und gehören in `002_fibu_600_tests.md`.
...
- **Schadens-Eskalation nach Betrag** (Spec 001_erp_300_prozesse 3): <50 → Stufe 1 (Kaution
