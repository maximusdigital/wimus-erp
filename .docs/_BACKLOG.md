...
|------|------------|------|---------|
| P2 | 2026-06-29 22:05 | ADMIN-/BERECHTIGUNGS-UI ausreifen (010_berechtigungen_400_design.md, existiert noch NICHT): Matrix A+B + Admin-Bereich sind in 010_300_prozesse konzeptuell da (Layout-Skizze, Scope-Umschalter, Bulk-Features, 2b Admin-Schutz), aber KEINE ausgereifte UI-Spec. Fehlt für „fertig": Screen-Aufbau /einstellungen/berechtigungen (Matrix A vs B Anordnung, Navigation im Einstellungs-Bereich); Interaktionsdetails (Scope-Umschalter-Widget, „Anna wie Ben"-Kopieren, Dirty-State+ein-Speichern-Flow, Validierungs-Feedback, Bestätigung bei Rechteentzug); Design-Spec nach WIMUS Design System (Token-Farben, Shadcn Table/Checkbox/Select, 4px-Grid); Empty/Loading/Error-States; MOBILE-Ansicht der Matrizen (390px — Matrix mit vielen Spalten ist knifflig, evtl. Karten-Alternative je Benutzer); DSGVO-Matrix-Export-UI (CSV-Button, Vorschau). BEWUSST NICHT JETZT: Scope-Spalten = firma/projekt werden erst durch #21 final → UI gegen bewegliche Achse ausspezifizieren = Nacharbeit. Ausarbeitung zusammen mit 010 Stufe 1 (nach #21 Phase B). Dann als 400_design-Spec + Bau-Prompt. | #24 |
